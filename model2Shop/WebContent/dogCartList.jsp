<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" 
    import="com.sun.org.apache.bcel.internal.generic.*, java.util.*, vo.*, java.text.SimpleDateFormat"%>
		...
		<c:if test="${cartList != null && cartList.size() > 0 }">
			...
			<form method="post">
				<table>
					...
					<tr class="tr_top">
						<th><input type="checkbox" id="allCheck" name="allCheck" onclick="checkAll(this.form)" /></td>
						<th>번호</th>
						<th>상품 이미지</th>
						<th>상품명</th>
						<th>가격</th>
					...
					<c:forEach var="cart" items="${cartList }" varStatus="status">
						<tr style="text-align: center;">
							...
							<td>${cart.price }</td>
							<td>
								<a href="dogCartQtyUp.do?kind=${cart.kind }" style="text-decoration: none; color:#333;">▲</a><br />
								${cart.qty }<br />
								<a href="javascript:checkQty('${cart.kind }', ${cart.qty })" style="text-decoration: none; color:#333;">▼</a>
							</td>
						</tr>
					</c:forEach>
					<tr>
